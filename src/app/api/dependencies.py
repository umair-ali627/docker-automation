...
from ..core.security import TokenType, verify_token, security
...
async def get_current_user(
    # CHANGED: The dependency now uses the new 'security' object
    credentials: Annotated[HTTPAuthorizationCredentials, Depends(security)],
    db: Annotated[AsyncSession, Depends(async_get_db)]
) -> dict[str, Any] | None:
    # ADDED: Extract the token string from the credentials
...
async def get_optional_user(request: Request, db: AsyncSession = Depends(async_get_db)) -> dict | None:
    ...
    try:
        ...
        # This will now correctly use the updated get_current_user logic if needed
        # We need to manually construct the credentials object for the dependency
